...
export class MqttConnect extends SharedConnect {
  ...
  constructor(
    config: { deviceId: string; deviceModel: string; debug?: boolean },
    openudid: string,
    eufyCleanApi: EufyLogin
  ) {
    super(config);
  ...
  public async connectMqtt(mqttCredentials): Promise<void> {
    if (mqttCredentials) {
      ...
      console.info("Setup MQTT Connection", {
        clientId: `android-${this.mqttCredentials.app_name}-eufy_android_${
          this.openudid
        }_${this.mqttCredentials.user_id}-${Date.now()}`,
      ...
      this.mqttClient = await mqtt.connect(
        "mqtt://" + this.mqttCredentials.endpoint_addr,
        {
          clientId: `android-${this.mqttCredentials.app_name}-eufy_android_${
            this.openudid
          }_${this.mqttCredentials.user_id}-${Date.now()}`,
  ...
  setupListeners() {
    this.mqttClient.on("connect", () => {
      ...
      console.debug(
        `Subscribe to cmd/eufy_home/${this.deviceModel}/${this.deviceId}/res`
      );
      this.mqttClient &&
        this.mqttClient.subscribe(
          `cmd/eufy_home/${this.deviceModel}/${this.deviceId}/res`
        );
  ...
  async sendCommand(dataPayload) {
    try {
      ...
      this.mqttClient.publish(
        `cmd/eufy_home/${this.deviceModel}/${this.deviceId}/req`,
        JSON.stringify(mqqtVal)
      );
